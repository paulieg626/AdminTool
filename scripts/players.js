...
function addUnit(table,b,dialog){
    ...
    table.button(new TextureRegionDrawable(Groups.player.index(b).icon()), Styles.clearNonei, 30, run(() => {})).size(40);
    ...
    bi2.clicked(() => {//Kill
        m.command("Groups.player.each(e => {if(e.id==\""+Groups.player.index(b).id+"\"){e.unit().kill()}})");
        dialog.hide();
    });
